...
class DQNtask :
    ...
    def __init__(self, systemPath: str, maxCost:int, batch_size:int,episodes:int,maxEpisodeLength:int,maxTraceLen:int):
        ...
        self.agent = MCTSAgent(self.env,maxCost, num_iterations=1000)
...
def main() -> int:
    ...
    # On charge la tâche avec ton JSON de voiture
    task=DQNtask("./jsons/ResilientCarMonV5.json",maxCost, batch_size=0, episodes=1, maxEpisodeLength=maxEpisodeLength, maxTraceLen=100)
    
    if task==None:
        return (1)
        
    print("Démarrage de la simulation d'attaque avec l'Agent MCTS...")
    
    # On utilise directement la méthode d'application existante
    task.application() 
